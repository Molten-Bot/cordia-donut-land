...
export function createCityItems() {
    ...
    for (let levelIndex = 0; levelIndex < maxLevel; levelIndex += 1) {
        ...
        for (let offset = 0; offset < 9; offset += 1) {
            ...
            items.push(makeItem(id, sectionStart + offset * 74 + (lane === 1 ? 18 : 0), laneY, radius));
            id += 1;
        }
    }
    return items;
...
function drawMascot(ctx, x, y, scale) {
    ctx.save();
    ctx.translate(x, y);
    ctx.scale(scale, scale);
    ctx.shadowColor = shadow;
...
function drawSceneCharacters(ctx, width, height, hole) {
